...
- **`system/`** is the host: window, keyboard, software PPU, APU synthesis.
  SDL2, native Win32, DOS, and terminal frontends all implement the same
...
platform.h
    ...
    +-- system/win95/      Win9x-compatible native GDI/WinMM host
...
`platform_apu_write`. SDL, Win95, and DOS turn those writes into PCM with
Nes_Snd_Emu.
...
4. Upload a 256×240 buffer (SDL texture, GDI DIB, VGA page, or ANSI
   cells).

The NES nametable persists in PPU RAM. SDL2 rebuilds its pixel image every
frame from that RAM, while the native Win95 adapter keeps a presentation-only
...
| Native Win95 / Win98 host | `system/win95/platform_win95.c` |
